<div @class(["w-full grid gap-20", "grid-cols-2" => \Cart::getContent()->count()])>
    <!-- Panel -->
    ...
    @else
        <div class="divide-y rounded-md text-gray-900">
        ...
        <div class="w-full mt-2 space-y-4">
            <form class="space-y-4">
                <x-input wire:model.defer="last_name" label="Nom" placeholder="Votre Nom" required />

                <x-input wire:model.defer="first_name" label="Prénom" placeholder="Votre Prénom" required />

                <x-input type="email" wire:model.defer="email" label="E-mail" placeholder="Votre adresse e-mail" required />

                <x-inputs.phone wire:model.defer="phone_number" label="Numéro téléphone" mask="['## ####-####']" placeholder="0 5/6/7/8 xx xx xx xx" required />

                <x-input wire:model.defer="address" label="Adresse" placeholder="Votre Adresse" required />
            </form>

            <button wire:click="checkout" class="btn btn-sm bg-black text-white">Checkout</button>
        </div>
    @endif
</div>
